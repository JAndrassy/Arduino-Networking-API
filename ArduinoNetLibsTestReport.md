...
* [WiFi.h include WiFiClient.h](https://github.com/arduino/ArduinoCore-renesas/pull/224)
...
* `disconnect` is asynchronous. Without waiting for status change after it, it can interfere with immediately following `begin` ()[PR](https://github.com/espressif/arduino-esp32/pull/9062))
...
* `config` has wrong ordering of parameters and doesn't have auto configuration for DNS, gateway, net mask for static IP ([PR](https://github.com/espressif/arduino-esp32/pull/8892))
...
* `config` has wrong ordering of parameters, no default auto values and doesn't return to DHCP with `config(INADDR_NONE)`
* `end()` crashes
...
* doesn't have `setHostname`. Mbed doesn't support hostname 
* can't return to DHCP after using static IP [PR](https://github.com/arduino/ArduinoCore-mbed/pull/791)
...
* [config(ip) doesn't set default DNS IP](https://github.com/arduino/ArduinoCore-renesas/pull/199)
* [wrong static DNS servers IP entries handling](https://github.com/arduino/ArduinoCore-renesas/pull/200)
* [static IP change not applied](https://github.com/arduino/ArduinoCore-renesas/pull/201)
